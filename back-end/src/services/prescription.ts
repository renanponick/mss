...
export default class PrescriptionService {
    ...
    async create(fields: CreatePrescription) {
        ...
        try {
            ...
            const newPrescription = await this.find(prescription.id)
            const result = await this.docusign.signEmbedded(newPrescription)
    ...
    async getPrescriptionsByToken(token: string) {
        ...
        return repository.query(`select r.*, p.name as patientName, d.name as doctorName, d.crx from prescription as r`
